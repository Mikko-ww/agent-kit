...
def save_learning(project_root: Path, entry: LearningEntry) -> Path:
    target_dir = learnings_dir(project_root)
...
def load_learning(project_root: Path, learning_id: str) -> LearningEntry | None:
    path = learnings_dir(project_root) / f"{learning_id}.jsonc"
    if not path.exists():
        return None
...
def list_learnings(project_root: Path) -> list[LearningEntry]:
    target_dir = learnings_dir(project_root)
...
def list_learning_ids(project_root: Path) -> list[str]:
    target_dir = learnings_dir(project_root)
    if not target_dir.exists():
        return []
    ids: list[str] = []
...
def save_rules(project_root: Path, rules: list[PromotedRule]) -> Path:
    path = rules_file(project_root)
    write_jsonc(path, {"rules": [r.to_dict() for r in rules]})
    return path


def load_rules(project_root: Path) -> list[PromotedRule]:
    path = rules_file(project_root)
    if not path.exists():
        return []
    try:
